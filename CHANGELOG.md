...
- fix(android): migrated to AGP's built-in Kotlin support following the [official plugin migration guide](https://docs.flutter.dev/release/breaking-changes/migrate-to-built-in-kotlin/for-plugin-authors), so the plugin keeps building on AGP 9+ where applying the Kotlin Gradle Plugin is no longer allowed ([#104](https://github.com/FlutterPlaza/no_screenshot/issues/104), [#110](https://github.com/FlutterPlaza/no_screenshot/pull/110)).
- fix(macos): replaced `CGWindowListCreateImage` (removed in the macOS 26 "Tahoe" SDK) with `cacheDisplay` for the blur-overlay snapshot, keeping the plugin compiling against the latest SDK without requiring Screen Recording permission ([#109](https://github.com/FlutterPlaza/no_screenshot/pull/109)). Because Flutter renders into a `CAMetalLayer` that AppKit snapshots cannot capture, the blur overlay on macOS now always uses the system blur material and `blurRadius` has no effect there; content remains fully obscured in Mission Control / the app switcher. Also fixes blur-overlay edges fading to transparent on iOS and macOS, thanks to @paulanatoleclaudot-betclic ([#102](https://github.com/FlutterPlaza/no_screenshot/pull/102)).
